...
__global__ void SoftmaxLossForwardGPU(const int nthreads,
          ...
          const bool weight_by_label_freqs, const Dtype* label_counts,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    // if (label_value >= num || label_value < 0) {
    //   LOG(FATAL) << "wrong label";
    // }
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      if (weight_by_label_freqs) {
        loss[index] *= label_counts[label_value];
      }
      counts[index] = 1;
    }
  }
...
__global__ void SoftmaxLossBackwardGPU(const int nthreads, const Dtype* top,
          ...
          const Dtype* label_counts, Dtype* bottom_diff,

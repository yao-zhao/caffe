...
__global__ void VanillaLadderCombinatorBackward(const int n,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    bottom_diff_z[index] = top_diff[index] * (weight_w0z[comb_index] +
      ...
      weight_wsigma[comb_index] * tempsig_data[index] *
      (1.-tempsig_data[index]) *
      (weight_w1z[comb_index] + weight_w1zu[comb_index] *
      bottom_data_u[index]));
    bottom_diff_u[index] = top_diff[index] * (weight_w0u[comb_index]+
      weight_w0zu[comb_index] * bottom_data_z[index] +
      weight_wsigma[comb_index] * tempsig_data[index] *
      (1.-tempsig_data[index]) *
      (weight_w1u[comb_index] + weight_w1zu[comb_index] *
      bottom_data_z[index]));
  }
}
...
__global__ void VanillaLadderCombinatorBackward_w(const int n,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    weight_diff_w0zu[comb_index] += top_diff[index] * bottom_data_z[index] *
    bottom_data_u[index];
    ...
    weight_diff_w1zu[comb_index] += tempsig_data[index] * bottom_data_z[index] *
    bottom_data_u[index];
  }
}
...
void VanillaLadderCombinatorLayer<Dtype>::Backward_gpu(
  ...
  // hack: use tempsig_ to hold temporary sig result,
  // tempsig = top_diff* wsig * Sig

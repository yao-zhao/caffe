...
__global__ void CyclicRollForward(const int n, 
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    for (int top_batch_rotid=0; top_batch_rotid<4; ++top_batch_rotid) {
        ...
        switch (rotation) {
          case 0: top_data[((top_batch_id*top_channels+top_channel_id)*size+h)*size+w]=bottom_data[index]; break;
          case 1: top_data[((top_batch_id*top_channels+top_channel_id)*size+size-1-w)*size+h]=bottom_data[index]; break;
          case 2: top_data[((top_batch_id*top_channels+top_channel_id)*size+size-1-h)*size+size-1-w]=bottom_data[index]; break;
          case 3: top_data[((top_batch_id*top_channels+top_channel_id)*size+w)*size+size-1-h]=bottom_data[index]; break;
          default: break;
       }
    }
  }
...
__global__ void CyclicRollBackward(const int n, 
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    for (int top_batch_rotid=0; top_batch_rotid<4; ++top_batch_rotid) {
        ...
        switch (rotation) {
          case 0: 
          bottom_diff[index] += top_diff[((top_batch_id*top_channels+top_channel_id)*size+h)*size+w]; 
          break;
          case 1: 
          bottom_diff[index] += top_diff[((top_batch_id*top_channels+top_channel_id)*size+size-1-w)*size+h]; 
          break;
          case 2: 
          bottom_diff[index] += top_diff[((top_batch_id*top_channels+top_channel_id)*size+size-1-h)*size+size-1-w];
          break;
          case 3: 
          bottom_diff[index] += top_diff[((top_batch_id*top_channels+top_channel_id)*size+w)*size+size-1-h];
          break;
          default: break;
       }
    }
...
void CyclicRollLayer<Dtype>::Forward_gpu(
  ...
  CyclicRollForward<Dtype>  
  <<<CAFFE_GET_BLOCKS(bottom_count), CAFFE_CUDA_NUM_THREADS>>>(bottom_count,

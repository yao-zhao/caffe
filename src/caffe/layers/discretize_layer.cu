...
void DiscretizeLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* separators_cpu_data = separators_.cpu_data();
  if (top.size() >= 2) {
    Dtype* centers_data = top[1]->mutable_cpu_data();
    centers_data[0] = separators_cpu_data[0] * Dtype(1.5) -
        separators_cpu_data[1] * Dtype(0.5);
    centers_data[num_separators_] =
        separators_cpu_data[num_separators_-1] * Dtype(1.5) -
        separators_cpu_data[num_separators_-2] * Dtype(0.5);
    for (int i = 0; i < num_separators_-1; ++i) {
      centers_data[i+1] =
          (separators_cpu_data[i] + separators_cpu_data[i+1]) / Dtype(2);
    }
  }
}
